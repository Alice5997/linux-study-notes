...
- 统一练习根目录：`~/shell_test`
- 脚本统一目录：`~/shell_test/scripts/`
- 日志练习数据目录：`~/shell_test/logs/`
...
ps aux | grep sleep
# 4.实时资源监控界面，q退出（任意目录执行）
top
# 5.优雅终止sleep进程，替换为自己查到的PID（任意目录执行）
kill 进程PID
# 6.重新创建后台进程用于强制杀死测试（任意目录执行）
sleep 300 &
# 7.强制杀死进程（任意目录执行）
kill -9 进程PID
# 8.按进程名批量终止（任意目录执行）
killall sleep
# 9.安装端口查看工具并查看监听端口（任意目录执行）
sudo dnf install -y net-tools
netstat -tunlp
# 10.使用ss查看端口（任意目录执行）
ss -tunlp
# 11.查看指定端口被哪个进程占用（任意目录执行）
ss -tunlp | grep 8080
```

【此处放置截图：ps aux、top、kill、killall 执行输出】
...
```
# 1.查看本机IP地址（远程连接要使用这个IP）
ip a
# 2.查看ssh服务状态
systemctl status sshd
# 3.设置ssh开机自启
sudo systemctl enable --now sshd
# 4.从本机远程登录另一台主机（示例：登录192.168.133.100）
ssh study@192.168.133.100
# 5.指定端口连接（SSH默认22，若改过端口）
...
# 10.切换到多用户命令行模式（老runlevel 3，任意目录执行）
sudo systemctl set-default multi-user.target
# 11.切换回图形界面模式（老runlevel 5，任意目录执行）
sudo systemctl set-default graphical.target
```

【此处放置截图：systemctl status sshd 输出】
【此处放置截图：enable/disable 与 is-enabled、get-default/set-default 输出】
...
```
# 1.查看crond服务状态（任意目录执行）
systemctl status crond
# 2.设置crond开机自启（任意目录执行）
sudo systemctl enable crond
# 3.编辑当前用户定时任务（首次选择编辑器选vim，任意目录执行）
crontab -e
# 4.查看当前用户定时任务（任意目录执行）
crontab -l
# 5.查看系统级任务（root，任意目录执行）
sudo crontab -l
```
...
完成脚本 `cleanup_logs.sh` + crontab 配置，验证任务可执行，操作过程记录到 `05_定时清理日志任务.md`（含脚本、crontab 配置、执行验证截图、避坑点）。
...
```
# 1.查看当前Shell解析器（任意目录执行）
echo $SHELL
# 2.查看系统可用解析器（任意目录执行）
cat /etc/shells
# 3.编写第一个脚本（在 ~/shell_test/scripts 目录执行）
...
```
# 4.赋予执行权限并运行（在 ~/shell_test/scripts 目录执行）
...
echo $version
# 8.位置参数脚本演示（在 ~/shell_test/scripts 目录执行）
vim args.sh
```
...
```
# 9.运行位置参数脚本（在 ~/shell_test/scripts 目录执行）
chmod +x args.sh
./args.sh hello shell
# 10.整数运算（任意目录执行）
echo $(( 3 + 5 ))
...
# 12.if判断脚本（在 ~/shell_test/scripts 目录执行）
vim if_demo.sh
```
...
```
# 13.运行if脚本三种情况（在 ~/shell_test/scripts 目录执行）
chmod +x if_demo.sh
./if_demo.sh 50
./if_demo.sh 150
./if_demo.sh
...
编写 `~/shell_test/scripts/filter_gov_log.sh`：接收日志文件路径参数，过滤 ERROR 日志，支持按关键字统计。完成后上传 GitHub 仓库 `Alice5997/linux-study-notes`。
...
编写 `~/shell_test/scripts/process_logs_batch.sh`：遍历 logs 目录所有 `.log` 文件，逐个统计 ERROR 条数并汇总报告。完成后上传 GitHub。
...
```
# 1.自定义函数与传参演示（在 ~/shell_test/scripts 目录执行）
vim func_demo.sh
```
...
```
# 2.运行函数脚本（在 ~/shell_test/scripts 目录执行）
...
# 6.sed文本替换（-i直接改文件，先预览再改，任意目录执行）
sed -i 's/old/new/g' ~/shell_test/logs/app.log
# 7.组合应用：提取ERROR日志中的时间列并排序（任意目录执行）
...
编写 `~/shell_test/scripts/log_funcs.sh`：把日志过滤、统计、备份封装成函数库，供其他脚本 source 调用。上传 GitHub。
...
编写 `~/shell_test/scripts/gov_log_auto.sh`，功能：
